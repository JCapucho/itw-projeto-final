...
let favorites = Object.entries(favoritesObj).map(([section, sectionData]) =>
...
function FavoritesViewModel() {
    ...
    self.unfavorite = function(entry) {
        ...
        favorites = favorites.filter(record =>
            record.Id != entry.Id || record.section != entry.section
        );
        self.favoritesEntries.remove(record =>
            record.Id == entry.Id && record.section == entry.section
        );
...
function search(term) {
    return favorites.filter(
        record => record.Name.toLowerCase().includes(term.toLowerCase())
    );
}
