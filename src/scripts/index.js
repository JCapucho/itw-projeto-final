const backupImages = [
  {
    ...
    description:
      "The XXXII Olympiad was held at Tokyo after a year of delay due to the COVID-19 pandemic",
  },
  {
    img: "assets/img/CarouselOlympic2016.jpg",
    name: "2016 Olympic Games",
    description:
      "The previous edition was held at Rio de Janeiro and featured over 11,000 athletes",
  },
  {
    img: "assets/img/CarouselOlympicWinter2022.webp",
    name: "Winter Olympic Games",
    description:
      "A separate event focused on winter sports last held in 2022 in Beijing",
  },
];
...
const factGenerators = [
  ...
  (data) => {
    const medalsCount = data.Medals.reduce(
      (accum, data) => accum + data.Counter,
      0
    );
...
async function loadGameImage(id) {
  ...
  if (data.Photo === null || data.Medals === null || data.Modalities === null)
    return null;
...
function IndexViewModel() {
  ...
  async function fetchGameImages() {
    ...
    imageFetcher: for (const game of shuffled) {
      while (inFlight.length + self.results().length >= 5) {
        ...
        inFlight = inFlight.filter(
          (queryPromise) => !queryPromise.isFulfilled()
        );
      }

      inFlight.push(
        MakeQueryablePromise(
          loadGameImage(game.Id).then((res) => {
            if (res === null) return;
