...
function filterData(sortedData, predicate) {
  ...
  for (let i = 0; i < sortedData.length; i++) {
    ...
    if (predicate(record)) {
      ...
      if (nextCheck && prevCheck) results.push(null);
    } else {
      results.push(null);
    }
  }
...
async function buildWinterSummerChart(chartId, title, endpoint) {
  ...
  const summerData = filterData(sortedData, (record) =>
    record.Name.includes("Summer")
  );
  const winterData = filterData(sortedData, (record) =>
    record.Name.includes("Winter")
  );
...
buildWinterSummerChart(
  "athletesPerGameChart",
  "Athletes per game",
  "Statistics/Games_Athletes"
);
buildWinterSummerChart(
  "competitionPerGameChart",
  "Competitions per game",
  "Statistics/Games_Competitions"
);
buildWinterSummerChart(
  "countryPerGameChart",
  "Countries per game",
  "Statistics/Games_Countries"
);
buildWinterSummerChart(
  "modalityPerGameChart",
  "Modalities per game",
  "Statistics/Games_Modalities"
);
