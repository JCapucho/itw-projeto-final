...
function buildGraph(athletes) {
  ...
  new Chart(document.getElementById("athletesPerCountryChart"), {
    type: "bar",
    data: {
...
function CountryViewModel() {
  ...
  async function loadAthletesInfo([country, game]) {
    ...
    const response = await fetch(
      `${API_URL}/Statistics/Athlete_Country?` + params
    );
  ...
  Promise.all([loadCountryInfo(), loadGameInfo()]).then(loadAthletesInfo);
}
